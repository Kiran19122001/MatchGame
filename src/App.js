...
class App extends Component {
  state = {
    activeFilterList: tabsList[0].tabId,
    timer: 60,
  ...
  componentDidMount() {
    this.timerId = setInterval(() => {
      this.setState(
        ({timer: prevTimer}) => ({timer: prevTimer - 1}),
        () => {
  ...
  playAgain = () => {
    this.setState({timer: 60, score: 0})
    this.componentDidMount()
  }

  changeImage = id => {
    const {displayImage} = this.state
    const result = imagesList.find(each => each.id === id)
    const comapareImages = result.imageUrl === displayImage
    if (comapareImages) {
      this.setState(prev => ({
        score: prev.score + 1,
      }))
      const changeImage = Math.floor(Math.random() * 30)
      this.setState({displayImage: imagesList[changeImage].imageUrl})
    } else {
  ...
  diplayActiveTab = tabId => {
    this.setState({activeFilterList: tabId})
  }

  updateTabId = () => {
    const {activeFilterList} = this.state
    const result = imagesList.filter(
      eachImage => eachImage.category === activeFilterList,
    )
    return result
  }

  render() {
    const {activeFilterList, timer, displayImage, score} = this.state
    const filteredImageList = this.updateTabId()
    return (
          ...
          <div className="b-container">
            <div className="top-container">
              ...
              <div className="right-container">
                <p className="paras">
                  Score: <span className="count-score">{score}</span>
                </p>
            ...
            <div className="score-card">
              <div className="game-over">
                <img
                  ...
                  className="trophy"
                />
                <p className="score-yr">Your Score</p>
                <p className="num-score">{score}</p>
          ...
          <div>
            ...
            <div className="image-container">
              <div className="display-image">
                <img src={displayImage} alt="match" className="d-image" />
              </div>
              <ul className="active-tab-list">
                {tabsList.map(eachTab => (
                  <ActiveItem
                    tabsList={eachTab}
                    key={eachTab.tabId}
                    diplayActiveTab={this.diplayActiveTab}
                    isActive={activeFilterList === eachTab.tabId}
                  />
                ))}
              </ul>
              <ul className="image-comp">
